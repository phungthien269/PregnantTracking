...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    ...
    console.error('[ai/chat] lỗi không mong muốn:', (err as Error).message)
    return apiOk({
      reply: 'Xin lỗi mẹ, hệ thống gặp sự cố tạm thời khi trả lời. Mẹ thử lại sau ít phút, hoặc tra cứu nhanh ở mục Thư viện / Cẩm nang nhé. Nếu câu hỏi liên quan sức khỏe khẩn cấp, mẹ liên hệ bác sĩ hoặc cơ sở y tế gần nhất.',
      sources: [],
      model: null,
      provider: null,
      ai: false,
      note: 'route error: ' + (err as Error).message,
      sessionId: null,
    })
  }
}
